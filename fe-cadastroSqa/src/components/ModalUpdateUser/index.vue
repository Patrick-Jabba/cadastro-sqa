...
import { GET_USERS, UPDATE_USER } from "../../store/type-actions";

const store = useStore();
const modal = useModal();

  const state = reactive({
    name: store.state.user.updateUser.name,
    cpf: store.state.user.updateUser.cpf,
    dataNascimento: fns.formatDateToViewWithDateInput(store.state.user.updateUser.dataNascimento),
    endereco: store.state.user.updateUser.endereco,
    sexo: store.state.user.updateUser.sexo,
  ...
  const getCurrentDate = computed(() => {
   ...
   async function handleSubmitForm() {
    try {
      ...
      state.user = {
        id: store.state.user.updateUser.id,
        name: state.name,
        cpf: state.cpf,
      ...
      const resultado = await store.dispatch(UPDATE_USER, state.user);
      if(!resultado) {
        await Swal.fire({
          position: "center",
          icon: "error",
      ...
      await Swal.fire({
          ...
          background: "#5061FC",
          title: `Sucesso!`,
          text: `Cadastro do usuário ${state.name} atualizado com sucesso!`,
          showConfirmButton: false,
          timer: 2500,
        });
